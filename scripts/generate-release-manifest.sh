...
BINARY_PATH="${BINARY_PATH:-$ROOT/target/$TARGET/release/kitsune-compositor}"
REPOSITORY="${REPOSITORY:-KitotsuMolina/Kito-compositor}"
TAG="${TAG:-v$VERSION}"
CHANNEL="${CHANNEL:-stable}"
COMMIT="${COMMIT:-$(git -C "$ROOT" rev-parse HEAD)}"
ARCHIVE_NAME="kitsune-compositor-$VERSION-$TARGET.tar.zst"
SBOM_NAME="kitsune-compositor-$VERSION-$TARGET.spdx.json"
MANIFEST_NAME="kitsune-compositor-$VERSION-$TARGET.manifest.json"
...
jq -n \
  ...
  '{
    ...
    payload: [
      ...
      {
        path: "share/licenses/kitsune-compositor/LICENSE",
        kind: "license",
        mode: "0644",
